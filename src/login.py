...
class Login:
    ...
    def __init__(self, ui):
        """constructor"""
        self.ui = ui
        self.frame = Frame(self.ui, bg='white')
    ...
    def login(self):
        ...
        cursor.execute(
            find_user, [(self.login_username.get()), (self.login_password.get())])
    ...
    def bridge_menu(self):
        ...
        for widget in self.ui.winfo_children():
            widget.destroy()
        menu.MenuApp(self.ui, self.login_username.get())
        self.ui.mainloop()
    ...
    def login_ui(self):
        ...
        label1 = Label(self.frame, text="Login to your account",
                       font=("", 27), pady=10, bg='white')
        label1.place(x=200, y=290)
        label2 = Label(self.frame, text="Username",
                       font=("", 17), pady=7, bg='white')
        label2.place(x=200, y=360)
        label3 = Label(self.frame, text="Password",
                       font=("", 17), pady=7, bg='white')
        label3.place(x=200, y=440)

        entry2 = Entry(self.frame, textvariable=self.login_username,
                       bd=5, font=("", 17), width=25)
    ...
    def register_ui(self):
        ...
        label1 = Label(self.frame, text="Create new account",
                       font=("", 27), pady=10, bg='white')
        ...
        label4 = Label(self.frame, text="Name",
                       font=("", 17), pady=7, bg='white')
        label4.place(x=200, y=380)
        label4 = Label(self.frame, text="Email",
                       font=("", 17), pady=7, bg='white')
        label4.place(x=200, y=460)
        label5 = Label(self.frame, text="Phone",
                       font=("", 17), pady=7, bg='white')
        label5.place(x=200, y=540)
        label6 = Label(self.frame, text="Address",
                       font=("", 17), pady=7, bg='white')
        label6.place(x=200, y=620)

        entry2 = Entry(self.frame, textvariable=self.username,
                       bd=5, font=("", 17), width=25)
        entry2.place(x=200, y=260)
        entry3 = Entry(self.frame, textvariable=self.password, bd=5, font=("", 17), width=25,
                       show="*")
        entry3.place(x=200, y=340)
        entry4 = Entry(self.frame, textvariable=self.name,
                       bd=5, font=("", 17), width=25)
        entry4.place(x=200, y=420)
        entry4 = Entry(self.frame, textvariable=self.email,
                       bd=5, font=("", 17), width=25)
        entry4.place(x=200, y=500)
        entry5 = Entry(self.frame, textvariable=self.phone,
                       bd=5, font=("", 17), width=25)
        entry5.place(x=200, y=580)
        entry6 = Entry(self.frame, textvariable=self.address,
                       bd=5, font=("", 17), width=25)
        ...
        label8 = Label(self.frame, text="Login", font=(
            "", 16), pady=10, fg="#668cff", bg='white')
